...
function tree_to_page_view($list,$level = 0,$repeat = "—&nbsp;")
{
    $data = '';
    foreach ($list as $key => $value) {
        $data = $data."<tr><td><input type='checkbox'  name='ids' ".get_checkbox_page_status($value['id'])." value='".$value['id']."'></td><td><a href='".url('edit',['id'=>$value['id']])."'>".str_repeat($repeat, $level).$value['title']."</a></td><td>".$value['name']."</td><td>".get_userinfo($value['uid'],'username')."</td><td>".date('Y-m-d H:i:s',$value['createtime'])."</td></tr>";
        if (!empty($value['_child'])) {
...
function tree_to_wx_menu($list,$level = 0,$repeat = "— —&nbsp;")
{
    $data = '';
    foreach ($list as $key => $value) {
        $data = $data."<tr><td>".str_repeat($repeat, $level)."<span class='editmenu' style='cursor:pointer' data='".$value['id']."'>".$value['name']."</span>"."</td></tr>";
        if (!empty($value['_child'])) {
