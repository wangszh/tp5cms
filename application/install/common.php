...
function register_administrator($db, $prefix)
{
    ...
    $sql = "INSERT INTO `[PREFIX]users` VALUES " .
           "('1','[UUID]','[NAME]', '[PASS]','[NICK]','[EMAIL]', ' ','[TIME]', '[IP]', '[SALT]','1',0,'','','')";
    //创建盐
    $salt = create_salt();
    //密码加密
    $password = minishop_md5($admin['password'],$salt);
    //创建UUID
    $uuid = create_uuid();
    $sql = str_replace(
        array('[PREFIX]','[UUID]', '[NAME]', '[PASS]', '[NICK]','[EMAIL]', '[TIME]', '[IP]','[SALT]'),
        array($prefix,$uuid, $admin['username'], $password,$admin['username'], $admin['email'],time(),$_SERVER["REMOTE_ADDR"], $salt),
        $sql);
    //执行sql
    $db->execute($sql);
    $sql = "INSERT INTO `[PREFIX]key_value` VALUES " .
           "('49','[COLLECTION]','[UUID]', '[NAME]','[VALUE]')";
    $sql = str_replace(
